...
export default function AliveMeter() {
  ...
  return (
    <div ref={targetRef} className="w-full text-center">
      ...
      <div className="mt-8">
        ...
        <div className="mt-2 h-4 w-full rounded-full bg-secondary">
          <motion.div
            className="relative h-full rounded-full bg-gradient-to-r from-accent/50 to-accent"
            style={{ width }}
          >
             <motion.div style={{ opacity: pulseOpacity }}>
                <Zap className="absolute -right-2 -top-2 h-8 w-8 text-yellow-400 fill-yellow-400 animate-pulse" />
             </motion.div>
          </motion.div>
        </div>
      </div>
